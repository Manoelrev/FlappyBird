...
function worldPos(){
    world_pos = world_pos - background_speed;
    if(world_pos < -600){world_pos = 0}
    background_pos = background_pos - (background_speed * 2);
...
function player_bird(){
    //Animation
    if(jump_player && animation_player < 2){pencil.drawImage(player_jump,player_x ,player_y - 25,70,50);}
    else if(animation_player >= 1 && animation_player <2){pencil.drawImage(player,player_x  ,player_y - 25,70,50);}
    else{pencil.drawImage(player_fall,player_x ,player_y - 25,70,50);}
...
document.onkeydown = function(event){
    if(jump_player == false){
        if(event.keyCode == 32){player_y = player_y - 100;jump_player = true; animation_player=0}}
